...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_special_event') {
    ...
    try {
        if ($id > 0) {
            $db->prepare("UPDATE special_events SET title=?, description=?, icon=?, badge_text=?, link_url=?, start_date=?, end_date=?, is_active=?, is_default=? WHERE id=?")
               ->execute([$title, $description, $icon, $badge_text, $link_url, $start_date, $end_date, $is_active, $is_default, $id]);
        } else {
            $db->prepare("INSERT INTO special_events (title, description, icon, badge_text, link_url, start_date, end_date, is_active, is_default) VALUES (?,?,?,?,?,?,?,?,?)")
               ->execute([$title, $description, $icon, $badge_text, $link_url, $start_date, $end_date, $is_active, $is_default]);
        }
      ...
      <form method="POST">
        ...
        <div class="form-group" style="margin-top:16px;">
          ...
          <label style="display:flex;align-items:center;gap:8px;font-weight:600;cursor:pointer;margin-top:8px;">
            <input type="checkbox" name="is_default" id="event_is_default" value="1">
            Is Default Event (Fallback)
          </label>
        </div>
...
function openEventModal() {
  ...
  document.getElementById('event_is_default').checked = false;
...
function editEvent(e) {
  ...
  document.getElementById('event_is_default').checked = e.is_default == 1;
  document.getElementById('event-modal').classList.add('open');
}
